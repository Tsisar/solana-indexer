...
func Start(ctx context.Context, db *storage.Gorm, resume bool, in <-chan string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to load signatures to parse: %w", err)
	}
	log.Infof("Found %d transactions to parse", len(signatures))
	...
	for {
		...
		case sig := <-in:
			if err := parseOneTransaction(ctx, db, true, sig); err != nil {
				return fmt.Errorf("failed to parse real-time transaction %s: %w", sig, err)
			}
		}
	}
}
...
func parseOneTransaction(ctx context.Context, db *storage.Gorm, resume bool, sig string) error {
	if resume {
		...
		if err != nil {
			return fmt.Errorf("failed to check if %s is parsed: %w", sig, err)
		}
		if parsed {
			log.Warnf("Transaction %s already parsed, skipping...", sig)
			return nil
		}
	}
	...
	if err != nil {
		return fmt.Errorf("failed to get raw transaction %s: %w", sig, err)
	}
	...
	if err := db.MarkParsed(ctx, sig); err != nil {
		return fmt.Errorf("failed to mark %s as parsed: %w", sig, err)
	}
