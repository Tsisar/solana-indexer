...
type StrategyReport struct {
	...
	Gain            types.BigInt            `gorm:"column:gain;default:0"`          // Reported gain amount (BigInt)
	Loss            types.BigInt            `gorm:"column:loss;default:0"`          // Reported loss amount (BigInt)
	CurrentDebt     types.BigInt            `gorm:"column:current_debt;default:0"`  // Reported current debt (BigInt)
	ProtocolFees    types.BigInt            `gorm:"column:protocol_fees;default:0"` // Reported protocol fees amount (BigInt)
	TotalFees       types.BigInt            `gorm:"column:total_fees;default:0"`    // Reported total fees amount (BigInt)
	TotalShares     types.BigInt            `gorm:"column:total_shares;default:0"`  // Reported total shares (BigInt)
...
func (s *StrategyReport) Init() {
	...
	s.TransactionHash = ""
	s.StrategyID = ""
	s.Gain = types.ZeroBigInt()
	s.Loss = types.ZeroBigInt()
	s.CurrentDebt = types.ZeroBigInt()
	s.ProtocolFees = types.ZeroBigInt()
	s.TotalFees = types.ZeroBigInt()
	s.TotalShares = types.ZeroBigInt()
	s.VaultKey = ""
}
